...
const API_URL = 'http://172.20.10.3:3000/su_kien';

export const createSuKien = async (formData) => {
  try {
...
export const getAllSuKien = async (token) => {
  try {
...
export const getByIdSuKien = async (id, token) => {
  try {
...
export const editSuKien = async (id, formData, token) => {
  try {
    const formDataToSend = new FormData();
    Object.entries(formData).forEach(([key, value]) => {
      formDataToSend.append(key, value);
...
export const removeSuKien = async (id, token) => {
  try {
