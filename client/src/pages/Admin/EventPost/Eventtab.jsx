...
const Eventtab = () => {
  ...
  const handleAddSuKien = () => {
      setEditingSuKien({ TenSK: "",NoiDung :"",MaTK:userId,TgianBatDau:"",TgianKetThuc:"",anh:"" });
  ...
  useEffect(() => {
      const fetchSuKien = async () => {
        try {
          ...
          setSuKienList(data);
          const datalist = await getAllSuKienData(token);
           ...
           <tbody>
             {filtered.map((row, index) => (
              <tr key={row.id}>
                ...
                <td>0</td>
                <td><button className='a' onClick={() => handleEditSuKien(row)}>✏️Sửa</button></td>
      ...
      {showDeleteConfirmSuKien && (
        <div className="modal">
          <div className="modal-content">
            ...
            <button className='a' onClick={confirmDeleteSuKien}>Xóa</button>
            <button className='a' onClick={cancelDeleteSuKien}>Hủy</button>
          </div>
        </div>
      )}
      {isEditModalOpenSuKien && (
        <div className="modal">
          <div className="modal-content">
            ...
            <button className='a' onClick={handleSaveEditSuKien}>Lưu</button>
            <button className='a' onClick={handleCloseEditSuKien}>Thoát</button>
          </div>
        </div>
      )}
      {isAddModalOpenSuKien && (
        <div className="modal">
          <div className="modal-content">
            ...
            <label>Ảnh:</label>
            <input type="file" onChange={(e) => setEditingSuKien({ ...editingSuKien, anh: e.target.files[0] })} /><br/>
            <button className='a' onClick={handleCreateSuKien}>Lưu</button>
            <button className='a' onClick={handleCloseAddSuKien}>Thoát</button>
          </div>
        </div>
      )}
        </div>
