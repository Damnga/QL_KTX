...
export const create = async (req, res, next) => {
  try {
    ...
    const DonXin = files?.DonXin?.[0]?.filename || null;
    const GiayXacNhanSinHVien = files?.GiayXacNhanSinHVien?.[0]?.filename || null;
    const CCCDPhoTo = files?.CCCDPhoTo?.[0]?.filename || null;
    const HopDong = files?.HopDong?.[0]?.filename || null;
    await createHoSo({DonXin, GiayXacNhanSinHVien, CCCDPhoTo, HopDong, MaHD});
...
export const update = async (req, res, next) => {
    try {
      ...
      await updateHoSo(id, {DonXin, GiayXacNhanSinHVien, CCCDPhoTo, HopDong, MaHD});
