...
import {getAllThongBao, getThongBaoById, createThongBao, updateThongBao, deleteThongBao } from "../model/ThongbaoModel.js"

export const create = async (req, res, next) => {
  try {
    const {NoiDung, Tgian, MaTK, MaPhong} = req.body;
    const anh = req.file?.filename || null;
    await createThongBao({NoiDung, Tgian, MaTK, MaPhong, anh});
    res.status(201).json({ message: 'Tạo thong bao thành công' });
  } catch (err) {
    next(err);
  }
};
...
export const update = async (req, res, next) => {
    try {
      ...
      const { NoiDung, Tgian, MaTK, MaPhong } = req.body;
      await updateThongBao(id, { NoiDung, Tgian, MaTK, MaPhong, anh});
