...
export const createThamGiaSuKien = async (hoadon) => {
  try {
    ...
    await db.query(
      'INSERT INTO dangkysukien (MaSV, MaSK, Tgian, TrangThai, GhiChu) VALUES (?, ?, ?, ?, ?)',
...
export const updateThamGiaSukien = async (id, newBaoTri) => {
  try {
    ...
    await db.query(
      'UPDATE dangkysukien SET MaSV = ?, MaSK = ?, Tgian = ?, TrangThai = ?, GhiChu = ? WHERE id = ?',
      [MaSV, MaSK, Tgian, TrangThai, GhiChu, id]
    );
  } catch (error) {
