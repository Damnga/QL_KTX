...
export const getLichSuRaVaoByIdSinhVien = async (id) => {
  try {
    const [rows] = await db.query('SELECT sinhvien.id,LoaiHoatDong,Tgian,TrangThai FROM lichsuravao,sinhvien  WHERE sinhvien.id=lichsuravao.MaSV AND  sinhvien.id = ?', [id]);
    return rows;
  } catch (error) {
    console.error('Lỗi khi lấy danh sách lich su ra vao:', error);
    throw error;
